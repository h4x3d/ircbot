...
module.exports = function(irc) {
  ...
  irc.use(route('!channels', auth.requiresUser, function(message) {
    Channel.find({}, function(err, channels) {
      if(err) {
        logger.error(err);
        irc.send(message.source, 'Something went wrong, check logs for more details');
        return;
      }

      _.pluck(channels, 'name').forEach(function(channel) {
        irc.send(message.source, channel);
      });
    });
  }));
